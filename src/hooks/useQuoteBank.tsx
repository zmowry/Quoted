...
import type { AdditionalQuotesSettings, NotificationTime, Quote, QuoteBankContextValue } from '@/src/types';
...
export function QuoteBankProvider({ children }: PropsWithChildren): JSX.Element {
  ...
  useEffect(() => { void (async () => {
    try {
      const [storedQuotes, storedTime, storedExtra] = await Promise.all([quoteStorage.getQuotes(), quoteStorage.getNotificationTime(), quoteStorage.getAdditionalQuotes()]);
      setQuotes(storedQuotes); setTime(storedTime); setAdditionalQuotes(storedExtra);
  ...
  const removeQuote = useCallback(async (id: string) => {
    ...
    setQuotes(next);
    await reconcileQueue(next);
    const current = next.some((quote) => quote.id === quoteOfDay?.id) ? quoteOfDay : await nextQuoteInCycle(next);
    setQuoteOfDay(current);
    await scheduleAllNotifications(notificationTime, current, additionalQuotes, next);
  }, [notificationTime, quoteOfDay, additionalQuotes]);

  const updateNotificationTime = useCallback(async (time: NotificationTime) => {
    await quoteStorage.setNotificationTime(time);
    setTime(time);
  ...
  const updateAdditionalQuotes = useCallback(async (settings: AdditionalQuotesSettings) => {
    await quoteStorage.setAdditionalQuotes(settings);
    setAdditionalQuotes(settings);
    await scheduleAllNotifications(notificationTime, quoteOfDay, settings, quotes);
  }, [notificationTime, quoteOfDay, quotes]);

  return <QuoteBankContext.Provider value={{ quotes, quoteOfDay, notificationTime, additionalQuotes, loading, saveQuote, removeQuote, refreshQuoteOfDay, updateNotificationTime, updateAdditionalQuotes }}>{children}</QuoteBankContext.Provider>;
}
